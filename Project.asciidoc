...
energymodel. One big deviation is that we need to artificially need to
know how far the receiver is to the sender-node, because the paper is
basing energy usage to distance. Read more on this in <<energy, energy section>>.
...
shortcut-routing<<mieghem>> i.e. forwarding using layer-2. Again, NS3's means of
routing are heavily influenced by classical IP networks -- it would've
forced us to implement our lower layers usuable by IP networks,
however the papers proposed network is based on a tree structure whose
routing rules are predefined.
...
physical layer -- which listens to actual bits sent and not calculate
energy usage by time, the latter is the case of
WifiRadioEnergyModel. The paper however expresses energy use in terms
of bits send over a certain distance following from the formula +
'C~ij~ = B~1~ + B~2~D^a^~ij~' + 

In reality this would mean that every node should be configured with
the knowledge how far its upstream node is located
